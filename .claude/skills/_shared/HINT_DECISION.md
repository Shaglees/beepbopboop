...
│   ├── Yes → display_hint: "calendar"  (and set scheduled_at)
│   └── No  → display_hint: "event"     (and set scheduled_at)
└── No → Is the post about a specific location (trail, cafe, venue, park)?
    ├── Yes → display_hint: "place"
    │        (evergreen recommendation; put any URL in body text —
    │         PlaceCard does not render external_url yet)
...
The server lint now warns in two places that map directly to this doc:
...
- `place_external_url_not_rendered` — you picked `place` and set `external_url`. The link won't render; inline it in the body or use `restaurant`/`destination`.

Both are warnings, not errors — your post still publishes — but treat them as errors in skill code until the client is updated.
